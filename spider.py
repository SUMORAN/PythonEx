...
driver2 = webdriver.PhantomJS(executable_path="F:\\phantomjs-2.1.1-windows\\bin\\phantomjs.exe")   
...
def getAbstract(num,title,url):  
    try:  
        ...
        result = codecs.open(fileName,'w','utf-8','ignore')   
        result.write("[Title]\r\n")  
        result.write(title+"\r\n\r\n")  
...
''''' 
 循环获取搜索页面的URL 
 规律 http://www.medlive.cn/pubmed/pubmed_search.do?q=pubmed&page=1 
'''  
def getURL():  
    page = 61      #跳转的页面总数  
    count = 918     #统计所有搜索的生物本体个数      
    while page<=65:  
        url_page = "http://www.medlive.cn/pubmed/pubmed_search.do?q=pubmed&page="+str(page)  
        print url_page  
        driver.get(url_page)  
